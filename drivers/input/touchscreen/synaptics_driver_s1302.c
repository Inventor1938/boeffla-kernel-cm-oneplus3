...
static void synaptics_ts_probe_func(struct work_struct *w)
{
	...
	TPD_ERR
	    ("boot_time: after optimize call [synaptics_ts_probe] on cpu %d\n",
	     smp_processor_id());
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (get_boot_mode() == MSM_BOOT_MODE__NORMAL) {
		...
		queue_delayed_work_on(i != NR_CPUS ? i : 0,
				      optimize_data.workqueue,
				      &(optimize_data.work),
...
static int synaptics_enable_interrupt(struct synaptics_ts_data *ts, int enable)
{
	...
	ret =
	    synaptics_rmi4_i2c_write_byte(ts->client, F01_RMI_CTRL01,
					  abs_status_int);
	if (ret < 0) {
...
{
	struct synaptics_ts_data *ts =
	    container_of(work, struct synaptics_ts_data, dwork.work);
	mutex_lock(&ts->mutex);
	synaptics_ts_report(ts);
	mutex_unlock(&ts->mutex);
...
static int synatpitcs_fw_update(struct device *dev, bool force)
{
	...
	if (ret < 0) {
		...
		ret =
		    synapitcs_ts_update(ts->client, fw->data, fw->size, force);
		if (ret < 0) {
			TPD_ERR
	...
	ret = synaptics_rmi4_i2c_write_byte(ts->client, 0xff, 0x0);
	ret =
	    synaptics_rmi4_i2c_read_block(ts->client, F34_FLASH_CTRL_BASE, 4,
					  buf);
	CURRENT_FIRMWARE_ID =
...
static int synaptics_s1302_fw_show(struct seq_file *seq, void *offset)
{
	...
	seq_printf(seq, "\
		...
		is update firmware state: %s\n", ts->manu_name, ts->fw_name, ts->fw_id, (ts->loading_fw) ? ("loading...") : ("no update"));
	return 0;
}

static ssize_t synaptics_s1302_fw_write(struct file *file,
					const char __user * page, size_t t,
					loff_t * lo)
{
	int val = 0;
	struct synaptics_ts_data *ts = tc_g;
...
static ssize_t synaptics_s1302_key_reverse_write(struct file *file,
						 const char __user * page,
						 size_t t, loff_t * lo)
{
	int ret = 0;
	char buf[10] = { 0 };
...
static int synaptics_s1302_key_reverse_open(struct inode *inode,
					    struct file *file)
{
	return single_open(file, synaptics_s1302_key_reverse_show,
			   inode->i_private);
}
...
static int synaptics_s1302_radd_show(struct seq_file *seq, void *offset)
{
	...
	seq_printf(seq,
		   "page:0x%x;address:0x%x;buff[%d]:[0]0x%x,[1]0x%x,[2]0x%x,[3]0x%x\n",
		   page, address, block, buffer[0], buffer[1], buffer[2],
		   buffer[3]);
	return 0;
}

static ssize_t synaptics_s1302_radd_write(struct file *file,
...
static ssize_t synaptics_s1302_reset_write(struct file *file,
...
static ssize_t synaptics_s1302_debug_write(struct file *file,
					   const char *buffer, size_t count,
					   loff_t * ppos)
{
	int ret, write_flag;
...
static ssize_t synaptics_s1302_wait_time_write(struct file *file,
					       const char *buffer, size_t count,
					       loff_t * ppos)
{
	int ret, write_flag;
...
static int synaptics_s1302_wait_time_open(struct inode *inode,
					  struct file *file)
{
	return single_open(file, synaptics_s1302_wait_time_show,
			   inode->i_private);
}
...
static void checkCMD(void)
{
	...
	do {
		...
		ret =
		    synaptics_rmi4_i2c_read_byte(tc_g->client,
...
static ssize_t tp_baseline_show(struct file *file, char __user * buf,
...
//      uint8_t tmp_new = 0;
	...
	if (ret < 0) {
		TPD_DEBUG
		    ("read_baseline: synaptics_rmi4_i2c_write_byte failed \n");
		//return sprintf(buf, "i2c err!");
	}

	ret =
	    synaptics_rmi4_i2c_write_byte(tc_g->client,
					  F54_ANALOG_CONTROL_BASE + 81, 0x01);
	ret =
	    synaptics_rmi4_i2c_read_byte(tc_g->client,
					 F54_ANALOG_CONTROL_BASE + 86);
	tmp_old = ret & 0xff;
	...
	ret =
	    synaptics_rmi4_i2c_write_byte(tc_g->client,
					  ...
					  (tmp_old & 0xef));
	ret =
	    synaptics_rmi4_i2c_write_word(tc_g->client, F54_ANALOG_COMMAND_BASE,
					  0x04);
	if (!(tc_g->using_polling)) {
	...
	for (x = 0; x < 2; x++) {
		...
		for (y = 0; y < 5; y++) {
			...
			if (x == 0
			    && y ==
			    3
			    /* && baseline_data[x][y] < 1500 && baseline_data[x][y] > 500 */
			    )
				num_read_chars +=
				    sprintf(&(kernel_buf[num_read_chars]),
					    "left:%d[0000,9999]\n",
					    baseline_data[x][y]);
			if (x == 0
			    && y ==
	...
	ret =
	    synaptics_rmi4_i2c_write_byte(tc_g->client, F54_ANALOG_COMMAND_BASE,
					  0X02);
	delay_qt_ms(60);
	...
	ret =
	    simple_read_from_buffer(buf, size, ppos, kernel_buf,
				    strlen(kernel_buf));
	kfree(kernel_buf);
	delay_qt_ms(10);
	if (tc_g->using_polling)
		schedule_delayed_work(&tc_g->dwork,
				      msecs_to_jiffies(polling_interval));
	else
		enable_irq(tc_g->irq);
	return ret;
...
static ssize_t synaptics_s1302_virtual_key_enable_write(struct file *file,
							const char __user *
							page, size_t t,
							loff_t * lo)
{
...
static int synaptics_s1302_virtual_key_enable_show(struct seq_file *seq,
						   void *offset)
{
...
static int synaptics_s1302_virtual_key_enable_open(struct inode *inode,
						   struct file *file)
{
	return single_open(file, synaptics_s1302_virtual_key_enable_show,
			   inode->i_private);
}
...
static int synaptics_s1302_proc(void)
{
	...
	proc_entry =
	    proc_create_data("fw_update", 0666, procdir, &proc_firmware_update,
			     NULL);
	proc_entry =
	    proc_create_data("key_rep", 0666, procdir, &proc_reverse_key, NULL);
	proc_entry = proc_create_data("radd", 0666, procdir, &proc_radd, NULL);
	proc_entry =
	    proc_create_data("reset", 0666, procdir, &proc_reset, NULL);
	proc_entry =
	    proc_create_data("debug", 0666, procdir, &proc_debug, NULL);
	proc_entry =
	    proc_create_data("strength", 0444, procdir, &proc_key_strength,
			     NULL);
	proc_entry =
	    proc_create_data("virtual_key", 0666, procdir, &proc_virtual_key,
			     NULL);
	proc_entry =
	    proc_create_data("touchkey_baseline_test", 0644, procdir,
			     &tp_baseline_image_proc_fops, NULL);
	proc_entry =
	    proc_create_data("setting_wait_time_test", 0644, procdir,
			     &setting_wait_time_proc_fops, NULL);
...
static int checkFlashState(struct i2c_client *client)
{
	...
	while ((ret != 0x80) && (count < 8)) {
		msleep(3);	//wait 3ms
		ret =
		    synaptics_rmi4_i2c_read_byte(client, SynaF34_FlashControl);
		count++;
	}
	if (count == 8)
		return 1;
			       ...
			       uint32_t data_len, bool force)
{
	...
	ret =
	    synaptics_rmi4_i2c_read_block(client, F34_FLASH_CTRL_BASE, 4, buf);
	CURRENT_FIRMWARE_ID =
	    (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
	FIRMWARE_ID =
	    (Config_Data[0] << 24) | (Config_Data[1] << 16) | (Config_Data[2] <<
							       8) |
	    Config_Data[3];
	...
	ret =
	    i2c_smbus_read_i2c_block_data(client, SynaF34ReflashQuery_BootID, 8,
					  &(bootloder_id[0]));
	TPD_DEBUG("bootloader id is %x \n",
		  (bootloder_id[1] << 8) | bootloder_id[0]);
	ret =
	    i2c_smbus_write_i2c_block_data(client, SynaF34Reflash_BlockData, 2,
					   &(bootloder_id[0x0]));
	TPDTM_DMESG
	...
	TPD_DEBUG("ret is %d\n", ret);
	re_scan_PDT(client);
	i2c_smbus_read_i2c_block_data(client, SynaF34ReflashQuery_BootID, 2,
				      buf);
	i2c_smbus_write_i2c_block_data(client, SynaF34Reflash_BlockData, 2,
				       buf);
	...
	for (j = 0; j < firmware; j++) {
		buf[0] = j & 0x00ff;
		buf[1] = (j & 0xff00) >> 8;
		synaptics_rmi4_i2c_write_block(client, SynaF34Reflash_BlockNum,
					       2, buf);
		synaptics_rmi4_i2c_write_block(client, SynaF34Reflash_BlockData,
					       16, &Firmware_Data[j * 16]);
