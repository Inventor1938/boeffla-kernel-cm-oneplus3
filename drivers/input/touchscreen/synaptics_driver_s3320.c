...
static void synaptics_get_coordinate_point(struct synaptics_ts_data *ts)
{
	...
	ret = synaptics_rmi4_i2c_write_byte(ts->client, 0xff, 0x4);
	ret =
	    i2c_smbus_read_i2c_block_data(ts->client, F51_CUSTOM_DATA11, 8,
					  &(coordinate_buf[0]));
	ret =
	...
	if (!memcmp
	    (coordinate_buf_last, coordinate_buf, sizeof(coordinate_buf))) {
...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	ret = i2c_smbus_write_byte_data(ts->client, 0xff, 0x00);
	ret =
	    i2c_smbus_read_i2c_block_data(ts->client, F12_2D_DATA04, 5,
					  &(gesture_buffer[0]));
	...
	if (version_is_s3508)
		TPD_DEBUG
		    ("s35080Gesture Type[0x%x]=[0x%x],lpwg Swipe ID[0x4%x] = [0x%x]\n",
	...
	case UNICODE_DETECT:
		gesture = (gesture_buffer[2] == 0x77) ? Wgestrue :
		    (gesture_buffer[2] == 0x6d) ? Mgestrue : UnkownGestrue;
	}
...
void int_touch(void)
{
	...
	ret = i2c_smbus_write_byte_data(ts->client, 0xff, 0x0);
	ret =
	    synaptics_rmi4_i2c_read_block(ts->client, F12_2D_DATA_BASE, 80,
					  buf);
	if (ret < 0) {
		TPD_ERR("synaptics_int_touch: i2c_transfer failed\n");
		goto INT_TOUCH_END;
					 ...
					 loff_t * ppos)
{
	...
	ret =
	    sprintf(page, "%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d\n",
		    gesture_upload, Point_start.x, Point_start.y, Point_end.x,
...
static ssize_t double_tap_enable_read_func(struct file *file,
					   char __user * user_buf, size_t count,
					   loff_t * ppos)
{
	int ret = 0;
...
static ssize_t double_tap_enable_write_func(struct file *file,
...
static ssize_t camera_enable_read_func(struct file *file,
				       char __user * user_buf, size_t count,
				       loff_t * ppos)
{
	int ret = 0;
...
static ssize_t camera_enable_write_func(struct file *file,
...
static ssize_t music_enable_read_func(struct file *file, char __user * user_buf,
				      size_t count, loff_t * ppos)
{
	int ret = 0;
	char page[PAGESIZE];
...
static ssize_t music_enable_write_func(struct file *file,
				       const char __user * buf, size_t count,
				       loff_t * ppos)
{
	int ret = 0;
...
static ssize_t flashlight_enable_read_func(struct file *file,
					   char __user * user_buf, size_t count,
					   loff_t * ppos)
{
	int ret = 0;
...
static ssize_t flashlight_enable_write_func(struct file *file,
					    const char __user * buf,
					    size_t count, loff_t * ppos)
{
	int ret = 0;
...
static ssize_t tp_baseline_show(struct device_driver *ddri, char *buf)
{
	...
	tmp_old = ret & 0xff;
	ret =
	    i2c_smbus_write_byte_data(ts->client, F54_ANALOG_CONTROL_BASE + 23,
				      (tmp_old & 0xef));
	ret =
	...
	tmp_new = ret & 0xdf;
	i2c_smbus_write_byte_data(ts->client, F54_ANALOG_CONTROL_BASE + 27,
				  tmp_new);
...
static int init_synaptics_proc(void)
{
	...
	prEntry_tmp =
	    proc_create("double_tap_enable", 0666, prEntry_tp,
			&double_tap_enable_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("camera_enable", 0666, prEntry_tp,
			&camera_enable_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("music_enable", 0666, prEntry_tp,
			&music_enable_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
	    proc_create("flashlight_enable", 0666, prEntry_tp,
			&flashlight_enable_proc_fops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
